...
/** Subscribe to storage changes (cross-tab support) */
...
/**
 * Hook to manage onboarding completion state via localStorage.
 * Uses useSyncExternalStore for consistent reads.
 */
export function useOnboarding() {
  ...
  const completeOnboarding = useCallback(() => {
    try {
      ...
      // localStorage unavailable — onboarding will replay next session
    }
  }, []);
